...
最近帮助了很多同学进行内推，作为一个有责任心的人，必须要全程帮大家查询内推的投递进度。
...
- 投递的同学无法及时获得反馈。（经常有人连续私信好几条，误以为是不是私信的格式不对）
- 私信积攒过多，我也容易眼花，依靠人工很容易出差错。
- 即使每天统一回复一次，几十甚至上百条的私信也耽误了我大量的休息时间。

毕竟身为一个程序员，那么能不能通过自动回复的方式，帮助我自动查询投递进度并发送给粉丝呢？
...
至此，我们**通过flask启动了一个web服务器**，并**通过NATAPP做内网穿透**，最后通过配置建立了**微信后台和本地服务器的链接**。环境调通，终于到了紧张刺激的写代码环节。
...
def weixin():
    if request.method == "GET":     # 判断请求方式是GET请求
        ...
        token = "XXXXXX"  # 一定要跟刚刚微信开发者设置中填写的token一致
        # 进行字典排序
...
![IMG_5118](https://gitee.com/guanlili1921/picturebed/raw/master/img/IMG_5118.PNG)
